...
//  参数说明      void       
//  返回参数      void
//  备注信息      设置系统时钟为792Mhz，并且设置PLL2和PLL3各个PFD时钟, 
//				   所有的时钟频率均按照I.MX6U官方手册推荐的值.
//-------------------------------------------------------------------------------------------------------------------
...
{
	imx6ul_hardfpu_enable();			//开启硬件浮点运算
	GIC_Init(); 						// 初始化GIC
	SystemInitIrqTable();				// 初始化中断表
	__set_VBAR((uint32_t)0x87800000); 	// 中断向量表偏移，偏移到起始地址 

	imx6u_clkinit();
	gpt_clock_delay_init();
	clk_enable();
}
